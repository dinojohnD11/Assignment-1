...
class UI{
    ...
    filterfunc(filter)// typed input value as asrgument 
    {
...
class LocalStore{
    ...
    static getieentriesfromLS() {
        ...
        if(localStorage.getItem('ieentries') === null) {
            ieentries = [];         //if there is no data in local storage(when the application is run for the first time)
        } else {
            ieentries = JSON.parse(localStorage.getItem('ieentries'));         
            for(let i=0;i<ieentries.length;i++) // upgrading the type of date from string to date
                {   
                    ieentries[i].date= new Date(ieentries[i].date);
                } 
        }   
        return ieentries;
    }
...
class Data{
    ...
    { 
        ...
        for(let i = 1; i < arr.length; i++) //validating each entry
        {
            ...
            if(Number(data[3]<0)) // checking if amount is negative
            {
...
{
    if(e.target.className=='delete') // ensuring the click is on the X
    {
    const ui = new UI();
...
{   //we are re displaying all the content in the list after deleting it
    ...
    if(ieentries.length==0){//verifying there is some data to be sorte
        ...
        }else if(sortby=='name')
        {
            ...
            ieentries.sort(function(a, b){return a.date - b.date});
